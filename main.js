...
function send(actionPayload) {
  if (!tg) {
    alert("Откройте это приложение внутри Telegram.");
    return;
  }
  tg.sendData(JSON.stringify(actionPayload));
  tg.showAlert("Запрос отправлен. Результат придёт сообщением в чат с ботом.");
}
...
modalForm.addEventListener("submit", (e) => {
  ...
  if (state.mode === "team") {
    ...
    renderTeams();
    // backend-less вариант: просим бота создать команду
    send({ action: "team_create", team_name: title });
    ...
    renderTasks();
    // Здесь team_id пока локальный. Поэтому действия с задачами делаем через team_id,
    // который пользователь берёт из сообщения "teams_list" и вставляет вручную (чуть хуже UX).
  }
...
// Быстрые действия через WebApp -> Bot
// Пользователь вводит team_id / join_key через prompt (чуть хуже UX, но работает без backend).
addTaskBtn.addEventListener("contextmenu", (e) => {
  e.preventDefault();
  const teamId = prompt("Введите team_id команды (из сообщения бота):");
  const description = prompt("Описание задачи:");
  if (!teamId || !description) return;
  const deadline = prompt("Дедлайн (любая строка, можно пусто):") || "";
  send({ action: "task_create", team_id: teamId, description, deadline });
});

window.addEventListener("keydown", (e) => {
  if (e.key === "F5") {
    e.preventDefault();
    send({ action: "teams_list" });
  }
});
